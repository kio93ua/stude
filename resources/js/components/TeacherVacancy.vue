  ...
  >
    ...
    <div class="mx-auto max-w-7xl px-6">
      ...
      <header ref="headerRef" class="mx-auto mb-10 max-w-3xl text-center">
        <p v-if="badgeText" class="badge-muted font-display inline-block">{{ badgeText }}</p>
        <h2 id="vacancy-title" class="heading-1 font-display tracking-tight text-secondary mt-4">
          {{ titleText }}
        </h2>
        <p v-if="subtitleText" class="mt-3 text-secondary/85">
          {{ subtitleText }}
        </p>
      </header>
        ...
        <div class="flex">
          ...
          >
            <div>
              ...
              <p v-if="leadText" ref="leadRef" class="mt-3 text-slate-700">
                {{ leadText }}
              </p>

              <!-- список з «пульсуючими» точками -->
              <ul v-if="bullets.length" ref="listRef" class="mt-6 grid gap-3">
                <li v-for="(b, i) in bullets" :key="`b-${i}`" class="flex items-start gap-3">
                  <span class="dot-pulse mt-1"></span>
                  <span class="text-slate-800">{{ b }}</span>
                </li>
              </ul>
            </div>
            ...
            <div class="mt-7">
              <a
                ...
                :aria-label="`${ctaText} — відкриється у новій вкладці`"
              >
                {{ ctaText }}
              </a>
...
type Props = {
  ...
  /** правий бейдж у картці («Чому з нами класно») */
  whyBadge?: string
  /** заголовок картки праворуч */
  cardTitle?: string
  /** вступний абзац у картці */
  lead?: string | null
  /** булети під лідом */
...
/* ===== нормалізація + fallback-и ===== */
const FALLBACKS = {
  badge: 'Вакансія',
  title: 'Запрошуємо вчителів англійської',
  subtitle: '',
  whyBadge: 'Чому з нами класно',
  cardTitle: 'Рости разом із школою — ми підтримуємо ініціативу',
  lead: 'Дамо свободу у методиці, сучасні матеріали та стабільний потік вмотивованих студентів. Ви зосереджуєтесь на викладанні — ми закриваємо операційку.',
  bullets: [
  ...
  mediaAlt: 'Ілюстрація викладача, який подає заявку',
  ctaText: 'Заповнити анкету',
  formUrl: 'https://docs.google.com/forms/d/e/1FAIpQLSew8oe-A0p3wS7omGT_u3h9ts04egW_Mr0SfIgYzXn8tQUekA/viewform?usp=header',
}

const trimStr = (v: unknown, fb = '') =>
  typeof v === 'string' ? (v.trim() || fb) : (typeof v === 'number' ? String(v).trim() || fb : fb)
...
/* секція */
const badgeText     = computed(() => trimStr(props.badge, FALLBACKS.badge))
const titleText     = computed(() => trimStr(props.title, FALLBACKS.title))
const subtitleText  = computed(() => trimStr(props.subtitle ?? '', ''))

/* картка праворуч */
const whyBadgeText  = computed(() => trimStr(props.whyBadge, FALLBACKS.whyBadge))
const cardTitleText = computed(() => trimStr(props.cardTitle, FALLBACKS.cardTitle))
const leadText      = computed(() => trimStr(props.lead ?? '', ''))
const bullets       = computed(() => toList(props.bullets, FALLBACKS.bullets))

/* медіа */
const mediaUrl      = computed(() => trimStr(props.mediaSrc, FALLBACKS.mediaSrc))
const mediaAlt      = computed(() => trimStr(props.mediaAlt, FALLBACKS.mediaAlt))

/* CTA */
const ctaText       = computed(() => trimStr(props.ctaText, FALLBACKS.ctaText))
const ctaHref       = computed(() => trimStr(props.formUrl, FALLBACKS.formUrl))
...
onMounted(() => {
  ...
  if (headerRef.value) {
    gsap.from(headerRef.value.children, {
      y: 18, opacity: 0, duration: 0.6, ease: 'power2.out', stagger: 0.06, immediateRender: false
    })
  }

  // ревіл для зображення
  if (mediaCard.value) {
    curtain.value = 1
  ...
  // права колонка — каскад
  if (cardRef.value) {
    const tl = gsap.timeline({
      defaults: { ease: 'power2.out', immediateRender: false },
      scrollTrigger: { trigger: cardRef.value!, start: 'top 80%', once: true }
    })
    tl.from([badgeRef.value, titleRef.value, leadRef.value], { y: 14, opacity: 0, duration: 0.6, stagger: 0.08 })
      .from(listRef.value?.children ?? [], { y: 10, opacity: 0, duration: 0.45, stagger: 0.06 }, '-=0.2')
      .from(ctaRef.value, { y: 10, opacity: 0, duration: 0.5 }, '-=0.15')
  }
...
.dot-pulse { @apply relative inline-block; width: 10px; height: 10px; }
.dot-pulse::before { content: ""; @apply absolute inset-0 rounded-full bg-teal-500/80; }
.dot-pulse::after { content: ""; @apply absolute inset-0 rounded-full bg-teal-500/30; animation: pulse 1.8s cubic-bezier(0.2, 0.7, 0.3, 1) infinite; }
